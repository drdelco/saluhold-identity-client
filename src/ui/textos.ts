...
export interface TextosUI {
  ...
  // ── Formulario de alta: lo que impide guardar ────────────────────────────
  ...
  // ── Botón de escanear el documento ───────────────────────────────────────
  escanearDni: string;
  subirFotos: string;
  ...
  // Ninguno culpa a quien hace la foto ni menciona la IA.
  ...
  // ── Faltas de configuración ──────────────────────────────────────────────
  // Son errores de quien monta el componente, pero acaban delante del usuario
  // por el mismo camino que los demás, así que también se traducen.
  errorEscanerNoConfigurado: string;
  errorQrNoConfigurado: string;
}

export const TEXTOS_UI_ES: TextosUI = {
...
let _textos: TextosUI = TEXTOS_UI_ES;
 ...
 */
export function configurarTextosUI(parcial: Partial<TextosUI>): void {
  const dados = Object.entries(parcial).filter(([, v]) => typeof v === 'string');
  _textos = { ...TEXTOS_UI_ES, ...(Object.fromEntries(dados) as Partial<TextosUI>) };
}

/** Los textos vigentes. Se llama al pintar, no al importar. */
export function textosUI(): TextosUI {
  return _textos;
}
...
export function interpolar(texto: string, vars: Record<string, string | number>): string {
  return texto.replace(/\{\{(\w+)\}\}/g, (marcador, campo: string) =>
    campo in vars ? String(vars[campo]) : marcador,
  );
}
