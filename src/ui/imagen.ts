...
const POR_DEFECTO: Required<Opciones> = {
  maxLado: 1600,
  calidad: 0.82,
  maxBytes: 1_100_000,   // el backend rechaza por encima de 1,5 MB
};

/** Decodifica respetando la orientación EXIF, con respaldo para navegadores viejos. */
async function decodificar(file: File): Promise<{ fuente: CanvasImageSource; ancho: number; alto: number }> {
...
function aBase64(dataUrl: string): string {
 ...
 */
export async function comprimirParaIA(file: File, opts: Opciones = {}): Promise<ImagenParaIA> {
  const { maxLado, calidad, maxBytes } = { ...POR_DEFECTO, ...opts };
  const { fuente, ancho, alto } = await decodificar(file);

  const dibujar = (lado: number, q: number): { base64: string; w: number; h: number } => {
  ...
  for (const [lado, q] of intentos) {
    ...
    if (bytesDeBase64(ultimo.base64) <= maxBytes) break;
  }
