...
const MOTIVO_POR_DEFECTO =
  'El documento no corresponde al paciente presente (verificación del documento físico).';
 ...
 */
export async function liberarNifEnIdentity(
  db: FirestoreMinimo,
  pacienteId: string,
  datos: DatosLiberacion,
  coleccionPacientes = 'pacientes',
  ...
  const clinicasPropietarias: string[] = Array.isArray(d.clinicaIds) ? d.clinicaIds as string[] : [];
