...
exports.canonizarIdentificador = exports.identificadorAceptable = exports.validarIdentificador = exports.tipoDeIdentificador = exports.normalizarIdentificador = void 0;
...
exports.normalizarIdentificador = normalizarIdentificador;
/**
 ...
 */
function tipoDeIdentificador(valor) {
...
exports.tipoDeIdentificador = tipoDeIdentificador;
...
function validarIdentificador(valor) {
    ...
    if (tipo === 'DNI') {
        ...
        if (letra === esperada)
            return { estado: 'valido', tipo: 'DNI', normalizado };
        return {
            estado: 'invalido',
            tipo: 'DNI',
            normalizado,
    ...
    if (tipo === 'NIE') {
        ...
        const esperada = letraDe(numero);
        if (letra === esperada)
            return { estado: 'valido', tipo: 'NIE', normalizado };
        return {
            estado: 'invalido',
            tipo: 'NIE',
            normalizado,
...
exports.validarIdentificador = validarIdentificador;
/** Atajo booleano para condiciones. `no-aplica` cuenta como aceptable. */
function identificadorAceptable(valor) {
    return validarIdentificador(valor).estado !== 'invalido';
}
exports.identificadorAceptable = identificadorAceptable;
/**
 ...
 */
function canonizarIdentificador(valor) {
    const v = normalizarIdentificador(valor);
    if (!v)
        return '';
    const dni = v.match(/^(\d+)([A-Z])$/);
    if (dni)
        return dni[1].padStart(8, '0') + dni[2];
    const nie = v.match(/^([XYZ])(\d+)([A-Z])$/);
    if (nie)
        return nie[1] + nie[2].padStart(7, '0') + nie[3];
    return v;
}
exports.canonizarIdentificador = canonizarIdentificador;
