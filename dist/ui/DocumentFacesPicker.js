...
export default function DocumentFacesPicker({ opened, onClose, onListo, origen }) {
    ...
    return (_jsx(Modal, { opened: opened, onClose: onClose, title: origen === 'camara' ? t.fotografiarDocumento : t.subirFotosDocumento, size: "md", centered: true, zIndex: 400, children: _jsxs(Stack, { gap: "sm", children: [_jsxs(SimpleGrid, { cols: 2, spacing: "sm", children: [_jsx(Hueco, { titulo: t.anverso, ayuda: t.ayudaAnverso, foto: anverso, origen: origen, onElegir: () => anversoRef.current?.click(), onQuitar: quitar(setAnverso) }), _jsx(Hueco, { titulo: t.reverso, ayuda: t.ayudaReverso, foto: reverso, origen: origen, onElegir: () => reversoRef.current?.click(), onQuitar: quitar(setReverso) })] }), _jsx(Text, { size: "xs", c: "dimmed", children: t.avisoFotoNoSeGuarda }), _jsxs(Group, { justify: "flex-end", children: [_jsx(Button, { variant: "subtle", color: "gray", onClick: onClose, children: t.cancelar }), _jsx(Button, { color: "serene", leftSection: _jsx(ScanLine, { size: 16 }), onClick: leer, disabled: !anverso, children: t.leerDocumento })] }), _jsx("input", { ref: anversoRef, ...inputProps, onChange: (e) => { asignar(setAnverso)(e.currentTarget.files); e.currentTarget.value = ''; } }), _jsx("input", { ref: reversoRef, ...inputProps, onChange: (e) => { asignar(setReverso)(e.currentTarget.files); e.currentTarget.value = ''; } })] }) }));
}
